      ...
      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          setProgress((prev) =>
            prev.map((p) =>
              p.name === file.name
                ? { ...p, progress: 100, status: "done", url: data.data.url }
                : p,
            ),
          );
          resolve(data.data);
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "#a0aec0", marginTop: 4 }}>
            Images (JPEG, PNG, WebP, GIF) and Documents (PDF, TXT, DOCX) · Max
            10MB each
          </div>
          <input
            ref={fileInputRef}
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: 13, color: "#718096" }}>
            Large files are split into 1MB chunks. If one fails, only that chunk
            needs to retry.
          </p>
          <input
            type="file"
          ...
          <div style={card}>
            ...
            {progress.map((p, i) => (
              <div key={i} style={{ marginBottom: 12 }}>
                ...
                <div
                  style={{ background: "#e2e8f0", borderRadius: 4, height: 8 }}
                >
                  <div
                    style={{
                      width: `${p.progress}%`,
            ...
            <div style={{ marginTop: 12 }}>
              {serverFiles.map((f, i) => (
                ...
                >
                  ...
                  <span
                    style={{ fontSize: 12, color: "#718096", marginRight: 12 }}
                  >
                    {f.size}
                  </span>
                  <a
