...
import { addItem, removeItem, updateQuantity, checkout, clearCart } from "./store/slices/cartSlice";
import {
  selectFilteredProducts, selectCategories, selectCartItemCount,
  selectCartSubtotal, selectCartWithDetails, selectCurrentUser,
  selectIsAuthenticated, selectProductsLoading, selectProductsError,
  selectCheckoutStatus, selectSelectedCategory
    ...
    <div style={{ fontFamily: "system-ui, sans-serif", background: "#f7fafc", minHeight: "100vh", padding: 24 }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h1 style={{ margin: 0, color: "#2d3748" }}>🛒 Day 10: Redux Toolkit</h1>
              ...
              <>
                ...
                <button onClick={() => dispatch(logout())}
                  style={{ padding: "6px 12px", borderRadius: 6, border: "none",
                    background: "#fed7d7", cursor: "pointer" }}>
                  Logout
                </button>
              </>
            ) : (
              <button onClick={() => dispatch(loginUser({ email: "alice@example.com", password: "pass" }))}
                style={{ padding: "6px 12px", borderRadius: 6, border: "none",
                  background: "#c6f6d5", cursor: "pointer" }}>
                Login (Demo)
              </button>
            )}
            <div style={{ background: "#4299e1", color: "#fff", borderRadius: "50%",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 20 }}>
          ...
          <div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                {categories.map((cat) => (
                  <button key={cat} onClick={() => dispatch(setSelectedCategory(cat))}
                    style={{ padding: "4px 12px", borderRadius: 20,
                      background: selectedCategory === cat ? "#4299e1" : "#e2e8f0",
                      color: selectedCategory === cat ? "#fff" : "#4a5568",
                      border: "none", cursor: "pointer", fontSize: 13, textTransform: "capitalize" }}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            {/* Products */}
            {isLoading && <div style={{ textAlign: "center", padding: 40 }}>Loading products...</div>}
            {productsError && (
              ...
              <div key={product.id} style={{ ...card, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: "#718096" }}>
                    {product.category} · ⭐ {product.rating} · {product.stock} in stock
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ fontWeight: 700, color: "#4299e1" }}>${product.price}</span>
                  <button
                    onClick={() => dispatch(addItem({ productId: product.id, name: product.name, price: product.price }))}
                    disabled={product.stock === 0}
                    style={{ padding: "6px 14px", borderRadius: 6, border: "none",
                      background: product.stock === 0 ? "#e2e8f0" : "#4299e1",
                      color: product.stock === 0 ? "#a0aec0" : "#fff", cursor: "pointer" }}>
          ...
          <div>
            <div style={card}>
              ...
              {cartItems.length === 0 ? (
                <p style={{ color: "#a0aec0", fontSize: 13 }}>Your cart is empty</p>
              ) : (
                <>
                  {cartItems.map((item) => (
                    <div key={item.productId} style={{ borderBottom: "1px solid #e2e8f0", padding: "8px 0" }}>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{item.name}</div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
                        <button onClick={() => dispatch(updateQuantity({ productId: item.productId, quantity: item.quantity - 1 }))}
                          style={{ padding: "2px 8px", borderRadius: 4, border: "1px solid #cbd5e0", cursor: "pointer" }}>−</button>
                        ...
                        <button onClick={() => dispatch(removeItem(item.productId))}
                          style={{ padding: "2px 6px", borderRadius: 4, border: "none",
                            background: "#fed7d7", cursor: "pointer", color: "#c53030" }}>✕</button>
                      </div>
                    </div>
                  ))}
                  <div style={{ marginTop: 12, paddingTop: 12, borderTop: "2px solid #e2e8f0" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, marginBottom: 12 }}>
                      <span>Total</span>
                      <span style={{ color: "#4299e1" }}>${cartSubtotal.toFixed(2)}</span>
                    </div>
                    <button
                      onClick={() => dispatch(checkout(cartItems))}
                      disabled={checkoutStatus === "loading"}
                      style={{ width: "100%", padding: "10px", borderRadius: 6, border: "none",
                        background: checkoutStatus === "loading" ? "#90cdf4" : "#48bb78",
                        color: "#fff", fontWeight: 700, cursor: "pointer" }}>
                      {checkoutStatus === "loading" ? "Processing..." : "Checkout"}
                    </button>
                    {checkoutStatus === "success" && (
                      <div style={{ marginTop: 8, padding: 8, background: "#c6f6d5",
                        borderRadius: 6, fontSize: 12, textAlign: "center" }}>
                        ✅ Order placed! <button onClick={() => dispatch(clearCart())}
                          style={{ border: "none", background: "none", color: "#276749",
            ...
            <div style={{ ...card, background: "#fffbeb", fontSize: 12 }}>
              ...
              <p style={{ margin: "4px 0 0" }}>
                Chrome Extension → Redux DevTools<br/>
