...
const products = Array.from({ length: 100 }, (_, i) => ({
  id: String(i + 1),
  name: `Product ${i + 1}`,
  price: parseFloat((10 + i * 2.5).toFixed(2)),
...
app.get("/health", async (_req, res) => {
  try {
    ...
    res
      ...
      .json({
        status: "error",
        redis: "disconnected — run: docker run -d -p 6379:6379 redis:alpine",
      });
  }
});
  ...
  async (_req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({
        ...
        hint: fromCache
          ? "Served from Redis (fast!)"
          : "Fetched from DB (slow), now cached",
      });
    } catch (err) {
      next(err);
    }
...
app.listen(3001, () => {
  ...
  console.log("\nEndpoints:");
  console.log(
    "  GET  /api/products        (cache-aside — first call slow, subsequent fast)",
  );
