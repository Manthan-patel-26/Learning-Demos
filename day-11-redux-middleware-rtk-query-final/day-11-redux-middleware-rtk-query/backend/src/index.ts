...
app.get("/api/products/:id", (req: Request, res: Response) => {
  ...
  if (!product) {
    res
      .status(404)
      .json({ status: "error", error: { message: "Product not found" } });
    return;
  }
...
app.post("/api/orders", (req: Request, res: Response) => {
  ...
  const total = items.reduce(
    (s: number, i: { productId: string; quantity: number }) => {
...
app.listen(3001, () => {
  ...
  console.log(
    "  GET  /api/products?page=1&limit=10&category=books&search=react",
  );
