...
function ProductCard({
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 12, color: "#718096", textTransform: "capitalize" }}
      >
      ...
      >
        ...
        <div style={{ display: "flex", gap: 4 }}>
          ...
          <button
            onClick={() =>
              updateProduct({
                id: product.id,
                changes: {
                  rating: parseFloat((product.rating + 0.1).toFixed(1)),
                },
              })
            }
            style={{
          ...
          <button
            onClick={() =>
              addToCart({ userId: USER_ID, productId: product.id })
            }
...
function CartPanel() {
  ...
  const [placeOrder, { isLoading: isCheckingOut, isSuccess }] =
    usePlaceOrderMutation();
  ...
  return (
    ...
    >
      <h3 style={{ margin: "0 0 12px" }}>
        🛒 Cart{" "}
        {isFetching && (
          <span style={{ fontSize: 11, color: "#a0aec0" }}>syncing...</span>
        )}
      </h3>
      {cartItems.length === 0 ? (
        ...
        <>
          {cartItems.map((item) => (
            ...
            >
              ...
              <button
                onClick={() =>
                  removeFromCart({ userId: USER_ID, productId: item.productId })
                }
                style={{
                  border: "none",
                  background: "#fed7d7",
            ...
            {["", "books", "electronics", "clothing"].map((cat) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  background:
                    category === (cat || undefined) ? "#4299e1" : "#e2e8f0",
                  color: category === (cat || undefined) ? "#fff" : "#4a5568",
                  fontSize: 13,
                }}
              >
                {cat || "All"}
        ...
        >
          <div>
            ...
            >
              ...
              {isFetching && (
                <span style={{ color: "#4299e1" }}>⟳ Fetching...</span>
              )}
              <span
                style={{ marginLeft: "auto", fontSize: 11, color: "#a0aec0" }}
              >
                Cache: all pages share one cache entry (serializeQueryArgs
                magic!)
              </span>
            </div>
            ...
            >
              {isFetching ? (
                ...
                <button
                  onClick={onLoadMore}
                  onMouseEnter={() =>
                    prefetchProducts({ page: page + 1, limit: 12, category })
                  }
                  style={{
                    padding: "8px 24px",
                    borderRadius: 20,
                ...
                data && (
                  <span style={{ color: "#a0aec0", fontSize: 13 }}>
                    All {data.pagination.total} products loaded
                  </span>
                )
              )}
            </div>
          ...
          <div>
            ...
            >
              ...
              <ul
                style={{ margin: "8px 0 0", paddingLeft: 16, lineHeight: 1.8 }}
              >
                <li>
                  <strong>Auto-caching:</strong> Same query → no refetch
                </li>
                <li>
                  <strong>Optimistic updates:</strong> Click ★+ to see instant
                  rating update (no server wait)
                </li>
                <li>
                  <strong>Cache invalidation:</strong> Add to cart → cart query
                  auto-refetches
                </li>
                <li>
                  <strong>Infinite scroll:</strong> serializeQueryArgs + merge
                </li>
                <li>
                  <strong>Prefetch:</strong> Hover a product → detail pre-loaded
                </li>
                <li>
                  <strong>Polling:</strong> Add pollingInterval: 30000 to any
                  query
                </li>
              </ul>
            </div>
