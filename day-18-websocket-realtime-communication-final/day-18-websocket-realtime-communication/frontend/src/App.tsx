...
export default function App() {
  ...
  const joinRoom = useCallback(() => {
    ...
    s.on(
      "user_joined",
      ({ user, onlineCount }: { user: OnlineUser; onlineCount: number }) => {
        setMessages((prev) => [
          ...prev,
          {
    ...
    s.on("typing", ({ users }: { users: string[] }) =>
      setTypingUsers(users.filter((u) => u !== name)),
    );

    s.emit("join", { name: name.trim(), room });
    // Get our userId from the first message we receive
    s.once("message", (msg: Message) => {
      if (msg.userId !== "system") setUserId(msg.userId);
    });
    ...
    return (
      ...
      >
        <div style={{ ...card, width: 360 }}>
          ...
          >
            {["general", "react", "nodejs", "devops"].map((r) => (
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border: "none",
              background: name.trim() ? "#4299e1" : "#e2e8f0",
              color: name.trim() ? "#fff" : "#a0aec0",
              fontWeight: 600,
      ...
      >
        ...
        <span
          style={{
            ...
            padding: "2px 8px",
            borderRadius: 10,
            background: isConnected ? "#48bb78" : "#fc8181",
          }}
        >
        ...
        {messages.map((msg) => {
          ...
          if (isSystem)
            return (
              <div
                key={msg.id}
                style={{ textAlign: "center", fontSize: 12, color: "#a0aec0" }}
              >
                {msg.text}
              </div>
            );
          return (
            ...
            >
              ...
              {isMe && msg.readBy.length > 1 && (
                <div style={{ fontSize: 10, color: "#4299e1", marginTop: 2 }}>
        ...
        {onlineUsers.map((u) => (
          <div
            ...
            style={{
              fontSize: 13,
              padding: "4px 0",
              color: u.id === userId ? "#4299e1" : "#4a5568",
            }}
          >
            ● {u.name}
            {u.id === userId ? " (you)" : ""}
