...
// ─── TYPES ────────────────────────────────────────────────
interface User { id: string; name: string; room: string; socketId: string; }
interface Message { id: string; userId: string; userName: string; room: string;
...
io.on("connection", (socket: Socket) => {
  ...
  socket.on("join", ({ name, room }: { name: string; room: string }) => {
    const user: User = { id: crypto.randomUUID(), name, room, socketId: socket.id };
    ...
    const onlineUsers = [...(rooms.get(room) ?? [])].map(sid => {
  ...
  socket.on("message", ({ text }: { text: string }) => {
    ...
    const message: Message = {
      ...
      userId: user.id, userName: user.name, room: user.room,
      text: text.trim().slice(0, 1000), // Limit message length
      timestamp: Date.now(), readBy: [user.id],
    };
    ...
    io.to(user.room).emit("typing", { users: [...(typingUsers.get(user.room) ?? [])] });
  });
  ...
  socket.on("typing_start", () => {
    ...
    socket.to(user.room).emit("typing", { users: [...(typingUsers.get(user.room) ?? [])] });
  });
  ...
  socket.on("read_messages", ({ messageIds }: { messageIds: string[] }) => {
    ...
    if (updated.length > 0) {
      io.to(user.room).emit("messages_read", { messageIds: updated, userId: user.id });
    }
  });
