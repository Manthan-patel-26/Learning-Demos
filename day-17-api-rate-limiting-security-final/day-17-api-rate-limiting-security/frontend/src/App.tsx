...
export default function App() {
  ...
  async function run(fn: () => Promise<ApiResult>) {
    ...
    try {
      ...
      setResult({
        status: 0,
        data: { error: "Backend not running. Run: cd backend && npm run dev" },
      });
    } finally {
      setLoading(false);
    }
      ...
      <div style={{ maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={card}>
          ...
          <p style={{ fontSize: 13, color: "#718096" }}>
            Click and check DevTools → Network → Response Headers
          </p>
          <button
            style={btn()}
            onClick={() => run(() => call("GET", "/health"))}
          >
            GET /health (check response headers!)
          </button>
          <button
            style={btn()}
            onClick={() => run(() => call("GET", "/api/security-headers"))}
          >
            View header list
          </button>
        </div>
        ...
        <div style={card}>
          <h3 style={{ marginTop: 0 }}>
            Rate Limiting — Auth Endpoint (max 10/15min)
          </h3>
          <p style={{ fontSize: 13, color: "#718096" }}>
            Click rapidly 11+ times to trigger the 429 rate limit
          </p>
          {[1, 2, 3, 4, 5].map((i) => (
            <button
              key={i}
        ...
        <div style={card}>
          ...
          <button
            style={btn("#e53e3e")}
            onClick={() =>
              run(() => call("POST", "/api/comments", { text: xssInput }))
            }
          >
          ...
          <button
            ...
            onClick={() =>
              run(() =>
                call(
                  "GET",
                  `/api/sql-injection-demo?search=${encodeURIComponent(sqlInput)}`,
                ),
              )
            }
          >
          ...
          >
            ...
            {result && (
              <span
                style={{
                  fontWeight: 700,
                  color: result.status >= 400 ? "#fc8181" : "#68d391",
                }}
              >
                HTTP {result.status}
              </span>
