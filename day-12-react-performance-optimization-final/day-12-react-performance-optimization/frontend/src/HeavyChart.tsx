...
interface Item { id: string; category: string; price: number; complexity: string; }
...
export default function HeavyChart({ items }: { items: Item[] }) {
  ...
  return (
    <div>
      <div style={{ background: "#c6f6d5", borderRadius: 8, padding: 10, marginBottom: 16, fontSize: 13 }}>
        ✅ This component was lazy-loaded! It wasn't in the initial JavaScript bundle.
        Open DevTools → Network → JS to see the separate chunk file that was downloaded.
      </div>

      <div style={card}>
        <h3 style={{ marginTop: 0 }}>📊 Category Distribution ({items.length.toLocaleString()} items)</h3>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {categoryData.map(([cat, count]) => (
            <div key={cat} style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 90, fontSize: 13, textAlign: "right", color: "#4a5568" }}>{cat}</div>
              <div style={{ flex: 1, background: "#e2e8f0", borderRadius: 4, height: 24, position: "relative" }}>
                <div style={{
                  background: "#4299e1", borderRadius: 4, height: "100%",
          ...
          {priceData.map(({ label, count }) => {
            ...
            return (
              <div key={label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                <div style={{ fontSize: 12, color: "#718096" }}>{count.toLocaleString()}</div>
                <div style={{ width: "100%", height, background: "#9f7aea", borderRadius: "4px 4px 0 0", transition: "height 0.5s" }} />
