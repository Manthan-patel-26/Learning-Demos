...
const items = Array.from({ length: 10000 }, (_, i) => ({
  ...
  complexity: ["easy","medium","hard","expert"][Math.floor(Math.random() * 4)],
  tags: Array.from({ length: Math.floor(Math.random() * 4) + 1 }, (_, j) =>
    ["react","typescript","node","sql","docker"][j % 5]
  ),
}));
...
app.get("/api/items/paginated", (req, res) => {
  ...
  res.json({
    ...
    total: items.length,
    page, totalPages: Math.ceil(items.length / limit),
  });
});

app.listen(3001, () => console.log("Day 12 backend on http://localhost:3001 (10k items at /api/items)"));
